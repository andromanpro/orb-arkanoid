...
- 🌐 **WebGL2 шейдер** для орбов (SDF, 2 слоя жидкости с Catmull-Rom, спекулярный блик) + **Canvas2D** для HUD, шарика, частиц, платформы
- 💧 **Жидкая физика** — 6-колоночная пружинная симуляция (Verlet), отклик на удар и взрывную волну соседей
- 🎵 **Процедурная музыка** через Web Audio API — 15 уровней со своей темой
- 📦 **Один файл `index.html`** — работает через `file://`, без зависимостей
...
Проверено — играется нормально, платформа следует за пальцем, коллизии и физика работают. **Единственное что не доделано** — нет автоматического полноэкранного режима, панели браузера остаются сверху. Как вариант — «Добавить на главный экран» через меню браузера.
...
| 1.0.0 | Полная игра: WebGL2, 15 уровней, бот, физика волн, музыка |
